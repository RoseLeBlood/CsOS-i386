...
#include <string.h>
#include <config.h>
#include <dev/net/NetworkDriver.hpp>
...
namespace dev
{
class Rtl8139Driver : public NetworkDriver
{
public:
    Rtl8139Driver();
    ~Rtl8139Driver();
...
private:
	...
	uint8_t   		m_version;
    uint8_t*  		m_TxBuffer;
    uint32_t 		m_TxBufferPhys;
    uint8_t   		m_TxBufferIndex;
    uint8_t*  		m_RxBuffer;
    uint32_t  		m_RxBufferPointer;
    uint32_t*     	m_MMIO_base;
private:
	static bus::pci_entry m_pciList[];
};
}
