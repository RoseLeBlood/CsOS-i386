...
#include <cxx/alignment.hpp>
#include <cxx/inttotype.hpp>
#include <cxx/iterator.hpp>
#include <cxx/type_traits.hpp>

namespace std
{
	template<typename T, size_t TCapacity> class fixed_list
	{
	    template<int i> struct type_selector
	    {
			typedef unsigned char   index_type;
	    };
	    template<> struct type_selector<1>
	    {
	  		typedef unsigned short  index_type;
	    };
	    template<> struct type_selector<2>
	    {
	...
	public:
	    ...
	    template<class InputIterator> fixed_list(InputIterator first, InputIterator last)
	    {
            get_root()->reset();
            assign(first, last);
	    }
